...
fn read_weather(time_idx: u32) -> WeatherSample {
	return weather_data[time_idx];
}

fn read_solar_exposure(point_idx: u32, time_idx: u32) -> f32 {
...
fn compute_outdoor_mrt(
	...
	let solar_exp: f32 = read_solar_exposure(point_idx, time_idx);
...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
	...
	let hours_per_day = max(params.hours_per_day, 1u);
	let day_start = (time_idx / hours_per_day) * hours_per_day;
	let day_end = min(day_start + hours_per_day - 1u, params.total_time_steps - 1u);
	...
	let averaged_utci = 0.5 * (utci0 + utci1);
	if (params.output_format == 0u) {
		output_utci[point_idx] = averaged_utci;
		return;
	}
