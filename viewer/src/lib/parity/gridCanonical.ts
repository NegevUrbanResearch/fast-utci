...
export function canonicalGridPoints(params: CanonicalGridParams): CanonicalGridResult {
	...
	for (let x = minX; x <= maxX + EPSILON; x += gridSize) {
		if (coordinateSystem === 'xy_ground') {
			// Python .bin reference ordering iterates analysis Y ascending.
			// Since viewer Z = -analysisY, we must iterate viewer Z descending
			// to preserve strict pointwise index alignment.
