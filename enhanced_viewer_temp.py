...
class EnhancedUTCIViewer:
    ...
    def visualize_enhanced_utci_heatmap(self, 
        ...
        if show_model_layers:
            self._add_model_layers(fig, enhanced_model, validation_csv)
        
        # Add UTCI data points if requested
        if show_utci_points:
            self._add_utci_points(fig, utci_results, point_size, enhanced_model, colorscale, colorscale_bounds, validation_csv)
    ...
    def _add_model_layers(self, fig: go.Figure, enhanced_model: EnhancedModel, validation_csv: str = None):
        ...
        for material_type, layers in layers_by_type.items():
            ...
            if combined_vertices:
                ...
                trace_kwargs = {
                    ...
                    'hoverinfo': "skip"  # Lower priority for model layers
                }
                
                # Add row/col parameters if using subplots
                if validation_csv and Path(validation_csv).exists():
                    trace_kwargs['row'] = 1
                    trace_kwargs['col'] = 1
    ...
    def _add_utci_points(self, fig: go.Figure, utci_results: Dict[str, Any], point_size: int, enhanced_model=None, colorscale=None, colorscale_bounds=None, validation_csv: str = None):
    ...
    def _add_comparison_table(self, fig: go.Figure, utci_results: Dict[str, Any], validation_csv: str, analysis_type: str):
        """Add comparison table with Grasshopper validation data using proper subplot layout."""
        try:
            ...
            # Add comparison table using proper subplot layout
    ...
    def create_animated_utci_heatmap(self, 
                                   enhanced_model: EnhancedModel,
                                   utci_results: Dict[str, Any],
                                   title: str = "Animated UTCI Thermal Comfort Heatmap",
                                   show_model_layers: bool = True,
                                   show_utci_points: bool = True,
                                   point_size: int = 8) -> go.Figure:
        """
        Create animated 3D visualization of UTCI heatmap with layer support.
        ...
        # Extract UTCI data points for animation
        positions = []
        utci_data_by_hour = {}
        ...
        for pos_key, data in utci_results.items():
            ...
            # Process UTCI values for each hour
            for hour_idx, utci_val in enumerate(utci_vals):
                if hour_idx not in utci_data_by_hour:
                    utci_data_by_hour[hour_idx] = []
                ...
                try:
                    ...
                    if not np.isnan(numeric_val):
                        utci_data_by_hour[hour_idx].append(numeric_val)
                    else:
                        utci_data_by_hour[hour_idx].append(None)
                except (ValueError, TypeError, AttributeError):
                    utci_data_by_hour[hour_idx].append(None)
        ...
        frames = []
        available_hours = sorted(utci_data_by_hour.keys())
        
        for hour in available_hours:
            utci_values = utci_data_by_hour[hour]
            
            # Filter out None values
            valid_indices = [i for i, val in enumerate(utci_values) if val is not None]
            if not valid_indices:
                continue
            
            valid_positions = positions[valid_indices]
            valid_utci = [utci_values[i] for i in valid_indices]
            ...
            hover_text = [
                ...
                f"UTCI: {utci:.1f}°C<br>"
                f"Hour: {hour}<br>"
                f"Comfort: {self.utci_colors.get_utci_abbrev(utci)}"
                for pos, utci in zip(valid_positions, valid_utci)
            ]
            
            # Add UTCI points trace for this hour
                    ...
                    go.Scatter3d(
                        ...
                        marker=dict(
                            ...
                            colorbar=dict(
                                title=dict(text="UTCI (°C)", side="right"),
                                tickmode="array",
                                tickvals=[self.colorscale_bounds[0] + i * (self.colorscale_bounds[1] - self.colorscale_bounds[0]) / 10 for i in range(11)],
                                ticktext=[f"{self.colorscale_bounds[0] + i * (self.colorscale_bounds[1] - self.colorscale_bounds[0]) / 10:.1f}" for i in range(11)]
                            )
                        ),
            ...
            sliders=[
                {
                    ...
                    'steps': [
                        {
                            'args': [[f"Hour {hour}"], {'frame': {'duration': 300, 'redraw': True}, 'mode': 'immediate'}],
                            'label': f"Hour {hour}",
                            'method': 'animate'
                        }
                        for hour in available_hours
